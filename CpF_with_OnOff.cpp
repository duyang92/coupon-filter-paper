...
uint32_t onoff_num_hash = 2;


void raw_onoff(vector<Pkt_Info> dataset, string res_path, uint32_t round_index) {
    OnOff_sketch on_off_ins(onoff_num_hash, total_memory / onoff_num_hash / (1 + sizeof(int32_t) * 8));
    ...
    for(; itr != res.end(); itr++) {
        if(itr->second > mea_th) {
            uint32_t ans_t = on_off_ins.query(itr->first);
            int diff = int(itr->second) - ans_t;
            if(diff < 0) diff = -diff;
            aae.push_back(diff / double(itr->second));
...
void cpf_with_onoff(vector<Pkt_Info> dataset, string res_path, uint32_t round_index) {
    OnOff_sketch on_off_ins(3, (total_memory - cpf_m * cpf_c) / 3 / (1 + sizeof(int32_t) * 8));
    CouponFilter cpf_ins(cpf_m, cpf_c, cpf_p, mea_tag);
    cpf_ins.init_update_interface(&on_off_ins);
    ...
    cout << "cpf+onoff: " << sum / aae.size() << endl;
}
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (strcmp(argv[i], "-onoff_num_hash") == 0)
        {
            string str_temp = string(argv[i + 1]);
            onoff_num_hash = strtoul(str_temp.c_str(), NULL, 10);
        }
    }
    ...
    if (eval_target.compare("raw_onoff") == 0) {
        ...
        for(int round_index = 1; round_index <= rounds; round_index++) {
            cpf_with_onoff(dataset, res_path, round_index);
        }
    }
}
